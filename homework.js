...
function getDaysAgo(timestamp) {
  ...
  const orderDay = today.diff(orderDate, "day");
  if (orderDay == 0) {
    return `今天`;
  } else {
    return `${orderDay}天前`;
...
function getThisWeekOrders(orders) {
  ...
  return orders.filter(function (order) {
    const orderDate = dayjs(order.createdAt * 1000);
...
function validateCartQuantity(quantity) {
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 99) {
    return { isValid: false, error: "必須是1~99的正整數" };
  }
  return { isValid: true };
}
...
1. _fetch要指定方法___________________________________

2. _fetch要設定 Content-Type_________________________

3. _fetch要手動轉 JSON 字串___________________________
*/
